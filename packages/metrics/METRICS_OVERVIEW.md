# Ritchy Metrics Implementation - Comprehensive Overview
...
- **Package**: Custom `@ritchy/metrics` package
...
Application Code
    ↓
@ritchy/metrics helpers (startEnrichmentTracking, trackExternalApiCall, etc.)
    ↓
...
- **Dashboard**: `packages/metrics/grafana/provisioning/dashboards/json/ritchy-overview.json`
...
|--------|------|--------|---------|
| `ritchy_http_requests_total` | Counter | method, route, status_code | Total request count |
| `ritchy_http_request_duration_seconds` | Histogram | method, route, status_code | Request latency (P50/P95/P99) |
| `ritchy_http_request_size_bytes` | Histogram | method, route | Request payload size |
| `ritchy_http_response_size_bytes` | Histogram | method, route, status_code | Response payload size |
...
| Metric | Type | Labels | Purpose |
|--------|------|--------|---------|
| `ritchy_enrichment_requests_total` | Counter | enrichment_type, status, cached | Success/failure count |
| `ritchy_enrichment_duration_seconds` | Histogram | enrichment_type, subprocess, cached | Duration tracking with sub-steps |
| `ritchy_enrichment_errors_total` | Counter | enrichment_type, error_type | Error categorization |
| `ritchy_enrichment_active` | Gauge | status | Active enrichments (processing/completed/failed) |
...
| Metric | Type | Labels | Purpose |
|--------|------|--------|---------|
| `ritchy_queue_jobs_processed_total` | Counter | queue_name, job_type, status | Job completion tracking |
| `ritchy_queue_job_duration_seconds` | Histogram | queue_name, job_type | Job processing time |
| `ritchy_queue_jobs_failed_total` | Counter | queue_name, job_type, error_type | Failed job tracking |
| `ritchy_queue_active_jobs` | Gauge | queue_name | Currently processing jobs |
...
| Metric | Type | Labels | Purpose |
|--------|------|--------|---------|
| `ritchy_database_queries_total` | Counter | operation, table | Query count by operation type |
| `ritchy_database_query_duration_seconds` | Histogram | operation, table | Query latency (P50/P95/P99) |
...
| Metric | Type | Labels | Purpose |
|--------|------|--------|---------|
| `ritchy_external_api_requests_total` | Counter | service, endpoint, status_code | API call tracking |
| `ritchy_external_api_duration_seconds` | Histogram | service, endpoint | API latency |
...
| Metric | Type | Labels | Purpose |
|--------|------|--------|---------|
| `ritchy_websocket_connections` | Gauge | namespace | Active WebSocket connections |
| `ritchy_websocket_messages_total` | Counter | namespace, event_type, direction | Message throughput |
...
| Metric | Type | Labels | Purpose |
|--------|------|--------|---------|
| `ritchy_nodejs_process_cpu_seconds_total` | Counter | - | CPU time consumed |
| `ritchy_nodejs_process_resident_memory_bytes` | Gauge | - | Resident memory size |
| `ritchy_nodejs_heap_size_total_bytes` | Gauge | - | Total heap size |
| `ritchy_nodejs_heap_size_used_bytes` | Gauge | - | Used heap size |
| `ritchy_nodejs_eventloop_lag_seconds` | Gauge | - | Event loop lag |
| `ritchy_nodejs_gc_duration_seconds` | Histogram | kind | Garbage collection duration |
...
# Average duration
rate(ritchy_enrichment_duration_seconds_sum[5m]) /
rate(ritchy_enrichment_duration_seconds_count[5m])
...
- **Example**: `ritchy_http_requests_total`, `ritchy_enrichment_duration_seconds`
...
curl http://localhost:3030/metrics | grep ritchy_enrichment_requests_total

# Query Prometheus directly
curl 'http://localhost:9090/api/v1/query?query=ritchy_http_requests_total'
```
...
sum(ritchy_queue_active_jobs) > 100
```
...
- **Grafana Dashboard**: http://localhost:3200/d/ritchy-overview
