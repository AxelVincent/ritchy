import { Badge } from '@/components/ui/badge'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { ChevronDown, ChevronRight, Users } from 'lucide-react'
import { useState } from 'react'
import { type Address, AddressDisplay } from './address-display'

export interface UBO {
  id?: string
  firstName?: string | null
  lastName?: string | null
  percentageOfShares?: string | number | null
  votingPercentage?: string | number | null
  nationality?: string | null
  country?: string | null
  dateOfBirth?: string | null
  address?: Address | null
}

interface UBOSectionProps {
  ubos: UBO[]
  showCard?: boolean
}

export const UBOSection = ({ ubos, showCard = true }: UBOSectionProps) => {
  const [expanded, setExpanded] = useState<Set<number>>(new Set())

  const toggleItem = (index: number) => {
    setExpanded((prev) => {
      const next = new Set(prev)
      if (next.has(index)) {
        next.delete(index)
      } else {
        next.add(index)
      }
      return next
    })
  }

  if (ubos.length === 0) return null

  const content = (
    <div className="space-y-1.5">
      {ubos.map((ubo, index) => {
        const isExpanded = expanded.has(index)
        const name = `${ubo.firstName || ''} ${ubo.lastName || ''}`.trim()
        const key = `ubo-${ubo.id || ''}-${ubo.firstName || ''}-${ubo.lastName || ''}-${index}`
        const hasDetails =
          ubo.nationality || ubo.dateOfBirth || ubo.country || ubo.address

        return (
          <div key={key} className="rounded-lg bg-muted/50 overflow-hidden">
            <button
              type="button"
              onClick={() => hasDetails && toggleItem(index)}
              className={`w-full flex items-center justify-between p-2.5 text-left ${hasDetails ? 'hover:bg-muted cursor-pointer' : 'cursor-default'} transition-colors`}
            >
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-sm font-medium">{name || 'Unknown'}</span>
                {ubo.percentageOfShares && (
                  <Badge variant="secondary" className="text-xs">
                    {ubo.percentageOfShares}% shares
                  </Badge>
                )}
                {ubo.votingPercentage && (
                  <Badge variant="outline" className="text-xs">
                    {ubo.votingPercentage}% voting
                  </Badge>
                )}
              </div>
              {hasDetails &&
                (isExpanded ? (
                  <ChevronDown className="h-4 w-4 shrink-0 text-muted-foreground" />
                ) : (
                  <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" />
                ))}
            </button>
            {isExpanded && hasDetails && (
              <div className="px-2.5 pb-2.5 pt-0 space-y-2 border-t border-border/50">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm pt-2">
                  {ubo.nationality && (
                    <p className="text-muted-foreground">
                      Nationality: {ubo.nationality}
                    </p>
                  )}
                  {ubo.country && (
                    <p className="text-muted-foreground">
                      Country: {ubo.country}
                    </p>
                  )}
                  {ubo.dateOfBirth && (
                    <p className="text-muted-foreground">
                      Date of Birth:{' '}
                      {new Date(ubo.dateOfBirth).toLocaleDateString()}
                    </p>
                  )}
                </div>
                {ubo.address && <AddressDisplay address={ubo.address} />}
              </div>
            )}
          </div>
        )
      })}
    </div>
  )

  if (!showCard) {
    return (
      <div className="space-y-2">
        <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
          Ultimate Beneficial Owners ({ubos.length})
        </p>
        {content}
      </div>
    )
  }

  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2">
          <Users className="h-4 w-4" />
          Ultimate Beneficial Owners ({ubos.length})
        </CardTitle>
      </CardHeader>
      <CardContent>{content}</CardContent>
    </Card>
  )
}
