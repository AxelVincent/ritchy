...
import { COMPANY_CREDITS } from './constants'
...
import { setCompanyEnrichmentStatus } from './status_manager'
  ...
  try {
    ...
    if (!website) {
      ...
      return {
        success: true,
        enrichmentId,
        message: 'Company enriched without website',
      }
    }
    ...
    if (isSocialMediaUrl(website)) {
      ...
      return {
        success: true,
        enrichmentId,
        message: 'Company enriched via social media profile',
      }
    }
