...
import { calculateContactCredits } from './calculate_contact_credits'
...
import { setContactEnrichmentStatus } from './status_manager'
...
export const contactEnrichmentService = async ({
  ...
  userId,
  reservedCredits,
}: ContactEnrichmentParams): Promise<ContactEnrichmentResult> => {
  ...
  try {
    ...
    if (contactRecord.enrichmentStatus === 'completed') {
      logger.info({
        msg: 'Contact already enriched, skipping',
      ...
      return {
        ...
        phonesFound: 0,
        creditsUsed: 0,
        creditsBreakdown: { linkedin: 0, emails: 0, phones: 0 },
      }
    }
